...
//!
//! ---- 1.17 (eldenring.exe 2.7.0.0) rebuild, 2026-09-03 ----
//! All RVAs below were re-derived against the 1.17 executable (AOB patterns from aobs_er.toml,
//! .pdata function starts, RIP-relative reference counting for the .data globals) and the struct
//! offsets were confirmed by disassembling the 1.17 area-BGM resolver (+0xb6/+0xf8 on FieldArea,
//! +0x5c28/+0x364 on WorldSoundMan, +0x435/+0x436/+0x2f0/+0x328 on CSSound). Only the area-id
//! comparison changed shape (top byte of the packed map id), mirrored in resolve_area_param_id.
...
const SETBOSSBGM_RVA: u32 = 0xdb4c20; // 1.17 (2.7.0.0); was 0xdb2ec0 in 1.16.2
const SETAREABGM_RVA: u32 = 0xdafd40; // 1.17; was 0xdadfe0 (AOB-verified, unique hit, .pdata function start)
const GLOBAL_FIELDAREA_RVA: u32 = 0x3d6d248; // 1.17; was 0x3d691d8 (fromsoftware-rs rva_ww + 248 code refs)
const GLOBAL_WORLDSOUNDMAN_RVA: u32 = 0x3d73778; // 1.17; was 0x3d6f708 (84 code refs; read by the area-BGM resolver)
...
unsafe fn resolve_area_param_id(cssound: usize, program: &Program) -> i16 {
    ...
    let mut area_param_id: i16 = if field_area_ptr.is_null() {
        ...
        // 1.17: the game reads the packed map id at FieldArea+0x2c through a tiny getter and
        // compares its top byte (the area) with 61:  cmp byte ptr [rax+3], 0x3d
        let map_id = *((field_area_ptr as usize + 0x2c) as *const u32);
        let area_id = (map_id >> 24) & 0xff;
        if area_id == 61 {
            if s == 999 {
                s = 50;
            }
